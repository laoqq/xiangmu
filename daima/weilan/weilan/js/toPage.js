 ...
 * @LastEditTime: 2019-08-29 01:47:15
...
function page_ctrl(data_obj) {
  ...
  var total_item = (data_obj.total_item !== undefined) ? Number(data_obj.total_item) : 0; //数据条目总数,默认为0,组件将不加载
  var per_num = (data_obj.per_num !== undefined) ? Number(data_obj.per_num) : 10; //每页显示条数,默认为10条
  var current_page = (data_obj.current_page !== undefined) ? Number(data_obj.current_page) : 1; //当前页,默认为1
  ...
  function page_even() {
    ...
    function change_content(arr) {
      ...
      if (localStorage.style == 'list1') {
        ...
        ht += $.map(arr.data, function (ele) {
          return `<div class="search-book-list bookList-text clearfix" data-id=${ele.bookid}> <a href="details.html" target="_blank">
                <div class="bookList-text-img bk-img-box"> <img alt="${ele.booktitle}"
                    src="${ele.bookimg}" style="height: 100%;"> </div> <em
                  class="bookList-text-title">${ele.booktitle}</em>
              </a>
              <p class="bookList-text-infor"><a href="details.html" target="_blank"> </a><a target="_blank"
                  href="###">${ele.bookauthor}</a>
                <span>著</span> <span class="bookList-text-publish">${ele.bookpublish}</span> <span
                  class="bookList-text-isbn">${ele.bookisbn}</span> <span class="bookList-text-time"${ele.booktime}
                  </span> </p>
              <p class="bookList-text-detail">
                ${ele.bookdetailbookprice}
              </p>
              <p class="bookList-text-price"> 蔚蓝价：<span class="red">${ele.price}</span> 定价：<del>
              ${ele.pricing}</del> <span
                  class="lightRed">省：<em>${ele.pricing - ele.price}</em></span> </p>
              <div>
                <p class="btn bookList-text-btn clearfix"> <a target="_blank" class="buy-btn" data-wlid="8648288"></a>
        ...
        ht += $.map(arr.data, function (ele) {
          return `<div class="search-book-item"> <a target="_blank" href="details.html">
    ...
    for (var i = 0; i < total_page - 1; i++) {
      ...
      } else if (total_page > 6 && current_page < total_page - 3 && i > current_page + 1) {
